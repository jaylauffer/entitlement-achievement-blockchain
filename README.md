...
The HTTP API is implemented with `actix-web`. All endpoints expect an `Authorization` header containing a developer token. Tokens are loaded at runtime from either a JSON file pointed to by the `DEVELOPER_TOKEN_FILE` environment variable or directly from the `DEVELOPER_TOKENS` environment variable containing JSON such as:

```json
{"dev1": "token1", "dev2": "token2"}
```
If no configuration is provided, the above sample tokens are used.
...
| `DEVELOPER_TOKEN_FILE` | Path to JSON file with developer tokens | _None_ |
| `DEVELOPER_TOKENS` | JSON map of developer tokens          | `{"dev1": "token1", "dev2": "token2"}` |
